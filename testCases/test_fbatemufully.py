...
class Testfbmcart:
    ...
    @pytest.mark.skipif(temu_fully_dict['loginpage'][0]['skip'] == True, reason='跳过执行')
    @pytest.mark.parametrize('CaseData', temu_fully_dict['loginpage'])
    def test_fba_temufully_paycart(self, shared_page, run_case_fixture, CaseData):
        run_case_fixture(shared_page, CaseData)
        try:
            shared_page.wait_for_timeout(1000)
